...
window.twistyjs = (function() {
	...
	twistyjs.TwistyScene = function() {
		...
		*/
		...
		this.rotateView = function(deltaTheta, deltaPhi, deltaRoll) {
			cameraTheta += deltaTheta || 0;
			cameraPhi += deltaPhi || 0;
			cameraRoll = (cameraRoll + (deltaRoll || 0)) % 48;
			moveCamera(cameraTheta, cameraPhi, true);
		}

		this.rotateViewMove = function(axis, pow) {
			var axes = {
		...
		function moveCameraDelta(deltaTheta, deltaPhi) {
			setViewDrag(cameraDragTheta + deltaTheta * Math.TAU / 48, cameraDragPhi + deltaPhi * Math.TAU / 48);
		}
		...
		function moveCamera(theta, phi, doRender) {
			...
			baseCameraOffset = new THREE.Vector3(xy * Math.sin(theta * Math.TAU / 48), z, xy * Math.cos(theta * Math.TAU / 48));
			baseCameraUp = new THREE.Vector3(0, 1, 0);
			applyDraggedCamera(doRender);
		}

		function ensureCameraVectors() {
			if (!baseCameraOffset) {
				baseCameraOffset = new THREE.Vector3().sub(camera.position, camera.target.position);
			}
			if (!baseCameraUp) {
				baseCameraUp = camera.up.clone();
			}
		}
		...
		function rotateVectorAroundAxis(vector, axis, angle) {
			...
			var cross = new THREE.Vector3().cross(axis, vector);
			return vector.clone().multiplyScalar(c)
				.addSelf(cross.multiplyScalar(s))
				.addSelf(axis.multiplyScalar(dot * (1 - c)));
		}

		function rotateCamera(axis, angle, doRender) {
			ensureCameraVectors();
			baseCameraOffset = rotateVectorAroundAxis(baseCameraOffset, axis, angle);
			baseCameraUp = rotateVectorAroundAxis(baseCameraUp, axis, angle).normalize();
			applyDraggedCamera(doRender);
		}
		...
		function applyDraggedCamera(doRender) {
			ensureCameraVectors();
			var offset = baseCameraOffset.clone();
			var up = baseCameraUp.clone();
			if (cameraDragTheta) {
				var yawAxis = new THREE.Vector3(0, 1, 0);
				offset = rotateVectorAroundAxis(offset, yawAxis, cameraDragTheta);
				up = rotateVectorAroundAxis(up, yawAxis, cameraDragTheta).normalize();
			}
			if (cameraDragPhi) {
				var forward = offset.clone().multiplyScalar(-1).normalize();
				var right = new THREE.Vector3().cross(up, forward);
				if (right.length() < 1e-6) {
					right = new THREE.Vector3(1, 0, 0);
				} else {
					right.normalize();
				}
				offset = rotateVectorAroundAxis(offset, right, cameraDragPhi);
				up = rotateVectorAroundAxis(up, right, cameraDragPhi).normalize();
			}
			cameraOffset = offset;
			cameraUp = up;
			applyCamera(doRender);
		}

		function applyCamera(doRender) {
			camera.position = new THREE.Vector3().add(camera.target.position, cameraOffset);
			camera.up = cameraUp.clone().normalize();
			if (doRender) {
				render();
			}
		}
		...
		function stopViewAnimation() {
			if (viewAnimationLoop !== null) {
				cancelRequestAnimFrame(viewAnimationLoop);
				viewAnimationLoop = null;
			}
		}

		function animateCameraRotation(axis, angle) {
			ensureCameraVectors();
			stopViewAnimation();
			var duration = 180 * Math.max(1, Math.abs(angle) / (Math.TAU / 4));
			var startTime = null;
			var prevEase = 0;
			function viewLoop(timeStamp) {
				...
				rotateCamera(axis, (ease - prevEase) * angle, true);
				prevEase = ease;
				if (progress < 1) {
					viewAnimationLoop = requestAnimFrame(viewLoop, twistyCanvas);
				} else {
					viewAnimationLoop = null;
				}
			}
